...
-- [FIX 3 + DELAY 20s + STARTUP 10s] Random Token
local function randomTokenStep()
    ...
    local now = os.clock()
    if now < randomToken.nextCheck then
        if randomToken.startupAt and now < randomToken.startupAt then
            randomToken.status = "Cho " .. math.ceil(randomToken.startupAt - now) .. "s khoi dong truoc khi random"
        end
        return
    end
    local character = player.Character
    ...
    if not rf or not rf:IsA("RemoteFunction") then
        randomToken.nextCheck = now + 20
        randomToken.status = "Chua tim thay GachaNetworkRF (cho 20s)"
        return
    end
    ...
    task.spawn(function()
        ...
        local ok, err = pcall(function()
            ...
            if not tokenReady then
                randomToken.nextCheck = os.clock() + 20
                randomToken.status = string.format("Token %d/%d (cho 20s)",
                    randomToken.tokenNow, randomToken.tokenNeed)
                return
            end
            if not randomToken.requirementMet then
                randomToken.nextCheck = os.clock() + 20
                randomToken.status = "Chua du dieu kien event (cho 20s)"
                return
            end
            if not cooldownReady then
                local wait = math.max(randomToken.cooldownLeft, 1) + 20
                randomToken.nextCheck = os.clock() + wait
                randomToken.status = string.format("Cooldown %ds + 20s (token %d/%d)",
            ...
            if accepted == false then
                randomToken.nextCheck = os.clock() + 20
                randomToken.status = "Server tu choi; Check lai sau 20s"
                return
            ...
            if ok2 and current() then
                ...
                randomToken.nextCheck = os.clock()
                    + math.max(randomToken.cooldownLeft, 1) + 20
                randomToken.status = string.format("Cooldown moi %ds + 20s; token %d/%d",
                    math.ceil(randomToken.cooldownLeft),
                    randomToken.tokenNow, randomToken.tokenNeed)
            else
                randomToken.nextCheck = os.clock() + 20
                randomToken.status = "Da quay; cho check lai sau 20s"
            end
        end)
        if not current() then return end
        if not ok then
            ...
            randomToken.nextCheck = os.clock() + 20
            randomToken.status = "Loi Check; thu lai sau 20s"
        end
...
local function renderDashboard()
    ...
    if randomToken.startupAt and os.clock() < randomToken.startupAt then
        refs.random.Text = "Random sau " .. math.ceil(randomToken.startupAt - os.clock()) .. "s"
    else
...
connect(player.CharacterAdded, function()
    ...
    if not randomToken.startupAt or os.clock() >= randomToken.startupAt then
        randomToken.nextCheck = 0
        randomToken.status = "Cho check lai sau respawn"
    end
end)
connect(RunService.Stepped, function()
